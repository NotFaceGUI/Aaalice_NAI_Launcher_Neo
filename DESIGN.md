...
无限画布是生成页中央工作区的第二种呈现，与图像预览在同一位置切换，桌面经典布局、官网式布局与移动端共用一套容器。

- **色面层级**：画布背景沿用 Canvas（`surface`）；节点是 Section 色面（`sectionSurfaceColor`），工具条与节点菜单是浮在画布上的 Section 色面；连线标签使用 Control 级色面。点阵背景只作空间参照，用低对比中性色，不使用渐变、纹理或装饰性发光。
- **节点状态**：静止时无描边、无投影；hover 使用与图像卡片一致的轻度内容交互阴影，只改变绘制不改变布局；选中与连线起点使用 2px primary 状态线画在上层，不占内容尺寸；连线手柄与缩放手柄只在 hover、选中或触屏呈现，不影响节点正文排版。
- **连线**：默认 `onSurfaceVariant`，选中 primary；标注文字与线宽随画布缩放，但在极小或极大缩放下夹在可读/可见区间。箭头只在连线完整呈现后出现。
- **输入**：平移与缩放读取指针与修饰键（Ctrl/⌘ + 滚动缩放，普通滚动平移），触屏用单指平移、双指缩放。hover 只作加速器：节点信息和完整提示词必须在选中或菜单里有等价入口；连接手柄在触屏常驻，不依赖 hover。
- **动效**：预览与画布水平位移切换、节点入场、连线生长、视口聚焦统一读取 `appTheme` 时长与曲线，只做单向减速，禁止回弹、过冲与缩放弹出；节点信息条只做 100–150ms 淡入与轻微上移。全部动效在 Reduce Motion 下立即到终态。
- **状态保持**：切换、断点变化与窗口最小化都不重置视口、选中与节点位置；关闭画布时保留画布子树而不是卸载。
